...
export default function ParentLogin() {
  ...
  return (
	<div className='parentlogin'>
		...
		<motion.h1 initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={()=>navigate("/")}>Parent Login</motion.h1>
		...
		<input required type="email" placeholder='Enter Email' onChange={(e)=>setEmail(e.target.value)} value={email}/>
			...
			<p style={{fontSize:"1.5rem"}}>Don't have an Account? <span onClick={()=>navigate('/parentregister')} style={{textDecoration:"underline",cursor:"pointer"}}>Register</span></p>
			<p style={{fontSize:"1.5rem"}}>Are you a Teacher? <span onClick={()=>navigate('/teacher')} style={{textDecoration:"underline",cursor:"pointer"}}>Login as Teacher</span></p>
		</motion.form>
		</div>
		}
